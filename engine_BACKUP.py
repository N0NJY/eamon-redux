...
class Engine:
    ...
    def _attempt_cast(self, spell_key: str, target_name: str = None) -> bool:
        ...
        if success_roll <= effective_prof:
            # SPELL SUCCEEDED
            if success_roll == 1:
                print(self.tc("CRITICAL SUCCESS!", "success"))

            # Attempt skill growth
    ...
    def _cast_blast(self, target_name: str = None) -> None:
        ...
        # Find target monster
        if target_name:
            target = self.world.find_monster_by_name(target_name, self.world.monsters_in_room(self.player.room_id))
        else:
        ...
        print(self.tc(f"A blast of pure energy hits {target.name} for {damage} damage!", "success"))
        ...
        # Monster dies?
        if target.hp <= 0:
            print(self.tc(f"{target.name} {target.death_message}", "success"))
            target.is_alive = False
            return
